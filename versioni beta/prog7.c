...
int main() {
    int ind1 = 0, ind2 = 0, check = 0, x, i,j;
    int max_testo = 100, max_lista_frasi = 100, max_lista_change = 100; // i max_testo si riferiscono reciprocamente a testo, lista_frasi, lista_change sono le dimensioni allocate a loro
    ...
    do {
        ...
        if (fp_lista_change > max_lista_change - 3) {//controllo sulla dimensione della struttura (numero di change delete si aggiorna sempre di 1)
...
char** ricomponi_testo(memory *dati, int fp_lista_change, char*** lista_change, char **testo, int *max_testo){
  ...
  new_testo=(char**)malloc((dati[fp_lista_change].fp_x-1)*sizeof(char*));
  for (j = i, l=0; j <=fp_lista_change ; j++) {
    for (k = 0; k <dati[j].dim ; k++,l++) {
      new_testo[l]=lista_change[j][k];
    }
  }
  *max_testo=dati[fp_lista_change].fp_x-1;
  free(testo);
  return new_testo;
